...
router.post('/cv', upload.single('cv'), async (req, res) => {
  ...
  try {
    ...
    if (bucket) {
      ...
      const fileId = uuidv4();
      const localDir = path.join(__dirname, '../../uploads/cvs', req.user.id);
      fs.mkdirSync(localDir, { recursive: true });
      fs.writeFileSync(path.join(localDir, `${fileId}.pdf`), req.file.buffer);
      cvUrl = `/uploads/cvs/${req.user.id}/${fileId}.pdf`;
      logger.info('Firebase não configurado — CV guardado localmente no disco', { userId: req.user.id });
    }
